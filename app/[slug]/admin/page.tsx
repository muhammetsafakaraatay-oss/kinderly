...
export default function AdminPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const navItems = [
    ...
    { id: 'ogrenciler', icon: '👦', label: 'Öğrenciler' },
    { id: 'siniflar', icon: '🏫', label: 'Sınıflar' },
    { id: 'yoklama', icon: '✅', label: 'Yoklama' },
    { id: 'aktivite', icon: '⚡', label: 'Aktiviteler' },
    { id: 'gunluk', icon: '📋', label: 'Günlük Rapor' },
    { id: 'gelisim', icon: '📈', label: 'Gelişim' },
    { id: 'fotograflar', icon: '📷', label: 'Fotoğraflar' },
    { id: 'aidat', icon: '💰', label: 'Aidat' },
    { id: 'yemek', icon: '🍽️', label: 'Yemek Listesi' },
    { id: 'personel', icon: '👨‍🏫', label: 'Personel' },
    { id: 'servis', icon: '🚌', label: 'Servis' },
    { id: 'mesajlar', icon: '💬', label: 'Mesajlar' },
    { id: 'duyurular', icon: '📢', label: 'Duyurular' },
    { id: 'etkinlikler', icon: '📅', label: 'Etkinlikler' },
    { id: 'ayarlar', icon: '⚙️', label: 'Okul Ayarları' },
  ]
          ...
          {navItems.map(item => (
            <button key={item.id} onClick={() => { setActivePage(item.id); setSidebarOpen(false) }}
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-all border-l-2 ${activePage === item.id
                ...
                : `border-l-transparent ${dark ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-50'}`}`}>
...
function Card({ children, dark, className = '' }: any) {
  return <div className={`rounded-lg border overflow-hidden ${dark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} ${className}`}>{children}</div>
}
...
function Modal({ open, onClose, title, children, dark }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className={`rounded-xl w-full max-w-lg max-h-screen overflow-y-auto ${dark ? 'bg-gray-800' : 'bg-white'}`}>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className={`text-xl font-bold ${dark ? 'text-white' : ''}`}>Hoş Geldiniz! 👋</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        {[
          { icon: '👦', label: 'Toplam Öğrenci', value: ogrenciler.length, color: 'text-indigo-600', page: 'ogrenciler' },
          { icon: '✅', label: 'Bugün Geldi', value: stats.geldi, color: 'text-teal-600', page: 'yoklama' },
          { icon: '💰', label: 'Geciken Aidat', value: stats.aidatKisi + ' kişi', color: 'text-orange-500', page: 'aidat' },
          { icon: '📅', label: 'Etkinlik', value: stats.etkinlik, color: 'text-indigo-600', page: 'etkinlikler' },
        ].map(w => (
          <div key={w.label} onClick={() => setActivePage(w.page)}
            className={`rounded-lg p-4 border cursor-pointer hover:shadow-sm transition-shadow ${dark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card dark={dark}>
          <div className={`px-4 py-3 border-b flex items-center justify-between ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
            <h3 className={`font-semibold text-sm ${dark ? 'text-white' : ''}`}>📅 Yaklaşan Etkinlikler</h3>
            <button onClick={() => setActivePage('etkinlikler')} className="text-xs text-indigo-600 font-medium">Tümü</button>
          </div>
          <div className="p-4">
            {events.length ? events.map(e => {
              const d = new Date(e.tarih)
              return <div key={e.id} className={`flex items-center gap-3 py-2 border-b last:border-0 ${dark ? 'border-gray-700' : 'border-gray-50'}`}>
                <div className="bg-indigo-600 text-white rounded-lg px-2 py-1 text-center min-w-10 flex-shrink-0">
                ...
                <div>
                  <p className={`text-sm font-semibold ${dark ? 'text-white' : ''}`}>{e.baslik}</p>
        ...
        <Card dark={dark}>
          <div className={`px-4 py-3 border-b flex items-center justify-between ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
            <h3 className={`font-semibold text-sm ${dark ? 'text-white' : ''}`}>💰 Geciken Aidatlar</h3>
            <button onClick={() => setActivePage('aidat')} className="text-xs text-indigo-600 font-medium">Tümü</button>
          </div>
          <div className="p-4">
            {debts.length ? debts.map((a, i) => (
              <div key={i} className={`flex items-center justify-between py-2 border-b last:border-0 text-sm ${dark ? 'border-gray-700' : 'border-gray-50'}`}>
                <span className={`font-medium ${dark ? 'text-white' : ''}`}>{(a.ogrenciler as any)?.ad_soyad || '—'}</span>
                <span className="text-red-500 font-semibold">{fmtM(a.tutar)}</span>
              </div>
            )) : <p className="text-sm text-teal-600 font-semibold">✅ Tüm aidatlar ödendi!</p>}
          </div>
        </Card>
    ...
    <div>
      <div className="flex gap-2 mb-4 flex-wrap">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Öğrenci ara..."
          className={`flex-1 min-w-48 border rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-500 ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300'}`} />
        ...
        <button onClick={openAdd} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">+ Ekle</button>
      </div>
            ...
            <tbody>
              {filtered.map((o: Ogrenci) => (
                <tr key={o.id} className={`border-t ${dark ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-100 hover:bg-gray-50'}`}>
                  ...
                  <td className="px-4 py-3"><span className="bg-indigo-100 text-indigo-700 text-xs font-semibold px-2 py-1 rounded-full">{o.sinif || '—'}</span></td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(o)} className="text-xs border border-gray-300 px-2 py-1 rounded hover:border-indigo-500 hover:text-indigo-600">✏️</button>
          ...
          return (
            <div key={s.id} className={`rounded-lg border overflow-hidden ${dark ? 'border-gray-700' : 'border-gray-200'}`}>
              ...
              <div className={`p-4 ${dark ? 'bg-gray-800' : 'bg-white'}`}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setEditing(s); setForm({ ...s }); setModal(true) }} className="flex-1 border border-gray-300 rounded-lg py-1.5 text-xs font-semibold hover:border-indigo-500 hover:text-indigo-600">✏️ Düzenle</button>
    ...
    <div>
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        ...
        <div className="ml-auto flex gap-4 text-sm font-semibold">
          <span className="text-teal-600">✅ {counts.geldi}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {filtered.map((o: Ogrenci) => {
          const d = state[o.id] || ''
          return (
            <div key={o.id} className={`border-2 rounded-xl p-3 text-center transition-all ${d === 'geldi' ? 'border-teal-400 bg-teal-50' : d === 'gelmedi' ? 'border-red-400 bg-red-50' : d === 'izinli' ? 'border-orange-400 bg-orange-50' : dark ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-white'}`}>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="max-h-80 overflow-y-auto">
            {filtered.map((o: Ogrenci) => (
              <div key={o.id} onClick={() => setSelected(o)}
                className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer ${selected?.id === o.id ? 'bg-indigo-50' : dark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={`${t?.emoji} ${t?.label}`} dark={dark}>
              ...
              <div className="flex gap-2 flex-wrap">
                {['Kahvaltı','Kuşluk','Öğle','İkindi'].map(v => (
                  <button key={v} onClick={() => setForm({...form, ogun: v})}
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium border ${form.ogun === v ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 text-gray-600'}`}>{v}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {['Hepsini','Çoğunu','Birazını','Hiç'].map(v => (
                  <button key={v} onClick={() => setForm({...form, yeme: v})}
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium border ${form.yeme === v ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 text-gray-600'}`}>{v}</button>
                ))}
              </div>
            </div>
          </>}
        ...
        <div className={`px-5 py-4 border-t flex justify-end gap-2 ${dark ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <button onClick={saveAkt} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">Kaydet</button>
        </div>
      </Modal>
    </div>
  )
  ...
  return (
    <div>
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        ...
        {selectedOgr && <button onClick={() => { setPuanlar({}); setGenelNot(''); setModal(true) }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">+ Rapor Ekle</button>}
      </div>
      ...
      {data.length > 0 && (
        <Card dark={dark}>
          ...
          {data.find(x => x.kategori === 'genel')?.not_text && (
            <div className={`mx-4 mb-4 p-3 rounded-lg border-l-4 border-indigo-500 ${dark ? 'bg-gray-700' : 'bg-indigo-50'}`}>
...
function Fotograflar({ siniflar, okul, dark }: any) {
  ...
  return (
    <div>
      ...
      <div className="flex justify-end mb-4">
        <button onClick={() => { setFiles([]); setForm({}); setModal(true) }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">+ Fotoğraf Ekle</button>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {fotos.map(f => (
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Fotoğraf Ekle" dark={dark}>
        ...
        <div className={`px-5 py-4 border-t flex justify-end gap-2 ${dark ? 'border-gray-700' : 'border-gray-200'}`}>
          <button onClick={() => setModal(false)} className="border border-gray-300 px-4 py-2 rounded-lg text-sm text-gray-600">İptal</button>
          <button onClick={upload} disabled={uploading} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold disabled:opacity-60">
    ...
    <div>
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        <input type="month" value={ay} onChange={e => setAy(e.target.value)}
          className={`border rounded-lg px-3 py-2 text-sm outline-none ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300'}`} />
        <button onClick={generate} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">📋 Ay Oluştur</button>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[['TOPLAM', toplam, 'text-indigo-600'], ['ÖDENEN', odenen, 'text-teal-600'], ['BEKLEYEN', toplam - odenen, 'text-red-500']].map(([l, v, c]) => (
            ...
            <tbody>
              {data.map(a => (
                <tr key={a.id} className={`border-t ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
                  ...
                  <td className="px-4 py-3">{a.odendi ? <span className="bg-teal-100 text-teal-700 text-xs font-semibold px-2 py-1 rounded-full">✅ Ödendi</span> : <span className="bg-red-100 text-red-600 text-xs font-semibold px-2 py-1 rounded-full">⏳ Bekliyor</span>}</td>
                  <td className={`px-4 py-3 text-xs ${dark ? 'text-gray-400' : 'text-gray-500'}`}>{a.odeme_tarihi || '—'}</td>
                  <td className="px-4 py-3">
                    {!a.odendi
                      ? <button onClick={() => markOdendi(a.id, a.tutar)} className="bg-teal-500 text-white text-xs px-3 py-1.5 rounded-lg font-semibold">✓ Ödendi</button>
                ...
                <tr key={r.tarih} className={`border-t ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => { setForm({ tarih: r.tarih, kahvalti: r.yemek.kahvalti || '', ogle: r.yemek.ogle || '', ikindi: r.yemek.ikindi || '' }); setModal(true) }}
                      className={`text-xs border px-2 py-1 rounded hover:border-indigo-500 hover:text-indigo-600 ${dark ? 'border-gray-600 text-gray-300' : 'border-gray-300'}`}>✏️</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function Personel({ siniflar, okul, dark }: any) {
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <button onClick={() => { setForm({ rol: 'ogretmen' }); setModal(true) }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">+ Personel Ekle</button>
      </div>
      <Card dark={dark}>
  ...
  return (
    <div>
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        ...
        <button onClick={save} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">💾 Kaydet</button>
        <div className="ml-auto flex gap-4 text-sm font-semibold">
          <span className="text-teal-600">🚌 {counts.bindi} Bindi</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {filtered.map((o: Ogrenci) => {
          const d = state[o.id] || ''
          return (
            <div key={o.id} className={`border-2 rounded-xl p-3 text-center transition-all ${d === 'bindi' ? 'border-teal-400 bg-teal-50' : d === 'indi' ? 'border-orange-400 bg-orange-50' : dark ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-white'}`}>
              <div className="text-2xl mb-1">🌸</div>
              <div className={`text-xs font-semibold mb-2 ${dark ? 'text-white' : ''}`}>{o.ad_soyad.split(' ')[0]}</div>
              <div className="flex gap-1 justify-center">
                {[['bindi','🚌 Bindi','bg-teal-500'],['indi','🏠 İndi','bg-orange-400']].map(([val,label,color]) => (
                  <button key={val} onClick={() => setState(s => ({ ...s, [o.id]: val }))}
                    className={`text-xs px-2 py-1 rounded font-bold transition-all ${d === val ? color + ' text-white' : 'bg-gray-100 text-gray-500'}`}>
                    {label}
                  </button>
      ...
      <Card dark={dark} className="flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {ogrenciler.map((o: Ogrenci) => {
            ...
            return (
              <div key={o.id} onClick={() => setSelected(o)}
                className={`flex items-center gap-3 px-4 py-3 cursor-pointer border-b ${dark ? 'border-gray-700' : 'border-gray-50'} ${selected?.id === o.id ? 'bg-indigo-50' : dark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                <div className="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center text-lg flex-shrink-0">🌸</div>
                <div className="flex-1">
                ...
                {unreadCount > 0 && <span className="bg-indigo-600 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{unreadCount}</span>}
              </div>
            )
          })}
        </div>
      ...
      <Card dark={dark} className="lg:col-span-2 flex flex-col overflow-hidden">
        {selected ? <>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {thread.map(m => (
              <div key={m.id} className={`flex ${m.gonderen_tip === 'okul' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${m.gonderen_tip === 'okul' ? 'bg-indigo-600 text-white rounded-br-sm' : dark ? 'bg-gray-700 text-white rounded-bl-sm' : 'bg-gray-100 rounded-bl-sm'}`}>
                  {m.icerik}
                  <div className={`text-xs mt-1 ${m.gonderen_tip === 'okul' ? 'text-indigo-200' : 'text-gray-400'}`}>{new Date(m.olusturuldu).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
              </div>
            ))}
          ...
          <div className={`p-3 border-t flex gap-2 ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
            ...
            <button onClick={send} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-semibold">➤</button>
          </div>
        </> : (
...
function Duyurular({ okul, dark }: any) {
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <button onClick={() => { setForm({}); setModal(true) }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">+ Duyuru Ekle</button>
      </div>
      <div className="space-y-3">
        {data.map(d => (
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Duyuru Ekle" dark={dark}>
        ...
        <div className={`px-5 py-4 border-t flex justify-end gap-2 ${dark ? 'border-gray-700' : 'border-gray-200'}`}>
          <button onClick={() => setModal(false)} className="border border-gray-300 px-4 py-2 rounded-lg text-sm text-gray-600">İptal</button>
          <button onClick={save} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">Yayınla</button>
        </div>
      </Modal>
    </div>
  )
...
function Etkinlikler({ siniflar, okul, dark }: any) {
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <button onClick={() => { setForm({}); setModal(true) }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">+ Etkinlik Ekle</button>
      </div>
      <div className="space-y-3">
        {data.map(e => {
          ...
          return (
            <div key={e.id} className={`rounded-lg border p-4 flex items-center gap-4 ${gecti ? 'opacity-60' : ''} ${dark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`${gecti ? 'bg-gray-400' : 'bg-indigo-600'} text-white rounded-xl p-3 text-center min-w-12 flex-shrink-0`}>
  ...
  return (
    <div className="max-w-lg">
      <Card dark={dark} className="p-6 space-y-4">
        ...
        <button onClick={save} className="w-full bg-indigo-600 text-white py-3 rounded-lg text-sm font-semibold">
